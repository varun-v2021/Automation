...
public class ScreenCastClientMain {

	static boolean debug = false;
    int port;
    
	public ScreenCastClientMain(int port, boolean debug) {
		this.port = port;
		this.debug = debug;
	}
    ...
    public static void startAgent(String debug1,String port1) {
		System.out.println("[agent] Starting ...");

		try {
    		//if(args.length == 0)
    			//throw new RuntimeException("Need >= 1 param");
    	    int port = Integer.parseInt(/*args[0]*/port1);
    	    //boolean debug = args.length >= 2 && args[1].equals("debug");
    	    boolean debug = true;
    	    new ScreenCastClientMain(port, debug).execute();
		} catch(Exception ex) {
			ex.printStackTrace();
		}
	}
